...
onValue(ref(db, "zebra"), (snapshot) => {
  ...
  let previousViolations = state.alerts;
  state.alerts = data.violations || 0;
  if (state.alerts > previousViolations) {
    addViolationImage();
  }
...
/*function renderViolationCards(violations) {
  ...
  violations.forEach((violation, index) => {
    ...
    const dateVal = violation.date || new Date().toLocaleDateString();
    const timeVal = violation.time || new Date().toLocaleTimeString();
    const violationIdVal = violation.id || `V-${Date.now() + index}`;

    // Generate details once
    const details = createViolationDetails(violationIdVal, dateVal, timeVal);
    card.violationDetails = details;

    card.innerHTML = `
      <div class="violation-image-container">
        ${imageHtml}
        <div class="violation-status-badge">${details.violationType.toUpperCase()}</div>
      </div>
      <div class="violation-content">
        <div class="violation-id">ID: ${violationIdVal}</div>
    ...
    card.addEventListener("click", () => {
      openViolationModal(card.violationDetails, imageSrc);
    });
...
if (burgerMenuBtn) {
  ...
  // Close menu when clicking outside (but exclude burger button itself)
...
const vehicleModels = {
  ...
  SUV: ["Mahindra XUV700", "Tata Harrier", "Toyota Fortuner", "Hyundai Tucson"],
  "Auto Rickshaw": ["Bajaj Compact RE", "Piaggio Ape", "Mahindra Treo"],
  Truck: ["Tata Prima", "Ashok Leyland Ecomet", "BharatBenz 2823C"],
  Bus: ["Volvo 9600", "Tata Starbus", "Ashok Leyland Viking"],
};
const indianNames = [
  "Rahul Sharma",
  "Amit Patel",
...
const insuranceStatuses = ["Valid", "Expired", "Not Insured"];
const challanAmounts = ["₹500", "₹1000", "₹2000", "₹5000"];
...
function createViolationDetails(violationId, date, time) {
  const vehicleType =
    vehicleTypes[Math.floor(Math.random() * vehicleTypes.length)];
  const models = vehicleModels[vehicleType];
  const vehicleModel = models[Math.floor(Math.random() * models.length)];

  // Generate a random Indian vehicle plate number (e.g. MH12AB1234)
  const states = ["MH", "DL", "KA", "HR", "GJ", "UP", "TS"];
  const randomState = states[Math.floor(Math.random() * states.length)];
  const randomDistrict = String(Math.floor(Math.random() * 90) + 10);
  const alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  const randomSeries =
    alphabet[Math.floor(Math.random() * 26)] +
    alphabet[Math.floor(Math.random() * 26)];
  const randomNumber = String(Math.floor(Math.random() * 9000) + 1000);
  const vehicleNumber = `${randomState} ${randomDistrict} ${randomSeries} ${randomNumber}`;
  ...
  const insuranceStatus =
    insuranceStatuses[Math.floor(Math.random() * insuranceStatuses.length)];
  const challanAmount =
    challanAmounts[Math.floor(Math.random() * challanAmounts.length)];
  const violationType =
...
function openViolationModal(details, imageUrl) {
  const modal = document.getElementById("violation-modal");
  if (!modal) return;

  const imgEl = document.getElementById("modal-vehicle-image");
  if (imgEl) {
    if (imageUrl) {
      imgEl.src = imageUrl;
  ...
  document.getElementById("modal-violation-id").textContent =
    details.violationId || "—";
  document.getElementById("modal-violation-type").textContent =
    details.violationType || "—";
  document.getElementById("modal-vehicle-number").textContent =
  ...
  document.getElementById("modal-owner-name").textContent =
    details.ownerName || "—";
  document.getElementById("modal-driver-name").textContent =
    details.driverName || "—";
  document.getElementById("modal-insurance-status").textContent =
    details.insuranceStatus || "—";
  document.getElementById("modal-challan-amount").textContent =
    details.challanAmount || "—";
...
function addViolationImage() {
  ...
  // 🔥 ESP32-CAM IP
  const imageUrl = "http://10.194.45.187/capture?t=" + Date.now();

  const now = new Date();
  const violationIdVal = `V-${Date.now()}`;
  const dateVal = now.toLocaleDateString();
  const timeVal = now.toLocaleTimeString();

  // Generate details once and store on the card
  const details = createViolationDetails(violationIdVal, dateVal, timeVal);
  card.violationDetails = details;

  card.innerHTML = `
    <div class="violation-image-container">
      <img src="${imageUrl}" class="violation-image" onerror="this.src='assets/no-image.png'">
      <div class="violation-status-badge">
        ${details.violationType.toUpperCase()}
      </div>
    </div>
  ...
  card.addEventListener("click", () => {
    openViolationModal(card.violationDetails, imageUrl);
  });
...
if (chartScrollContainer && scrollIndicator) {
  ...
  chartScrollContainer.addEventListener("scroll", () => {
    // Hide immediately on scroll
    scrollIndicator.classList.add("hidden");

    // Clear previous timeout and wait for scrolling to stop
    clearTimeout(scrollTimeout);
    scrollTimeout = setTimeout(() => {
      updateScrollIndicator();
    }, 500); // Re-evaluate after 500ms of inactivity
  });

  // Touch listener to hide indicator during active swiping
  ...
  // Initial check on resize/load
  window.addEventListener("resize", () => {
    updateScrollIndicator();
  });

  // Run initially
  setTimeout(updateScrollIndicator, 600);
}
